        ...
        <div className="border border-black dark:border-white p-8 flex flex-col gap-8">
            ...
            <div className="flex flex-wrap gap-4 pt-4 border-t border-black dark:border-white">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    ...
                    <button onClick={() => setConfirm("reassign")} className="px-4 py-2 border border-black dark:border-white text-sm hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition">
                        Reassign
                    </button>
                </div>
            </div>
            ...
            {confirm == "reassign" && (
                <div onClick={() => setConfirm(null)} className="fixed inset-0 bg-black/50 flex items-center justify-center">
                    <div onClick={e => e.stopPropagation()} className="bg-white dark:bg-black border border-black dark:border-white p-6 max-w-md w-full rounded-lg">
                        ...
                        <p className="text-sm text-gray-500 mb-6">
                            Unassigned handlers. Select one.
                        </p>

                        <div className="flex flex-col items-center justify-center gap-4 mb-6">
                            {unassignedHandlers !== null ? unassignedHandlers?.map(handler => (
                                <button className={`rounded-md px-4 py-3 ${handlerId == handler.id ? "bg-white text-black border-2 border-black" : "bg-black text-white"} w-3/4`} onClick={() => setHandlerId(handler.id)}>{handler.name}</button>
                            )) : (<p>No unassigned handlers at this point.</p>)}
                        </div>
